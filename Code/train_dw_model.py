def train_model_gev(model_name=None, lambda_=None, lambda_2=None, config=None, checkpoint_dir=None, tuning=False,
                    validation=True, hetero="h", final_train=False, X_train_h=X_train_h, X_train_m=X_train_m,
                    X_train_mask=X_train_mask, y_train=y_train):
    if tuning:
        if model_name == "LSTM_max":
            model = LSTM_max(n_features_h, sequence_len_h, batch_size, config["n_hidden"], config["n_layers"])
        # todo: tune for other models
        optimizer = torch.optim.Adam(model.parameters(), lr=config["lr"])
    else:
        if model_name == "M1_GEV":
            model = M1_GEV(input_size, batch_size, n_hidden_h)
        elif model_name == "FCN":
            model = FCN(sequence_len_h, batch_size, n_hidden_h)
        elif model_name == "M2_GEV":
            model = M2_GEV(n_features_h, sequence_len_h, batch_size, n_hidden_h, n_layers)
        elif model_name == "LSTM_GEV":
            model = LSTM_GEV(n_features_h, n_features_m, sequence_len_h, sequence_len_m, batch_size, n_hidden_h,
                             n_hidden_m, n_layers)
        elif model_name == "DeepPIPE":
            model = DeepPIPE(n_features_h, n_features_m, sequence_len_h, sequence_len_m, batch_size, n_hidden_h,
                             n_hidden_m, n_layers)
        elif model_name == "PM1":
            model = PM1(n_features_h, n_features_m, sequence_len_h, sequence_len_m, batch_size, n_hidden_h, n_hidden_m,
                        n_layers)
    ...
    train_loader = DataLoader(TensorDataset(X_train_h, X_train_m, X_train_mask, y_train), batch_size=batch_size,
                              worker_init_fn=seed_worker)
    if validation:
        validation_loader = DataLoader(TensorDataset(X_val_h, X_val_m, X_val_mask, y_val), batch_size=batch_size,
                                       worker_init_fn=seed_worker)
        test_loader = DataLoader(TensorDataset(X_test_h, X_test_m, X_test_mask, y_test), batch_size=batch_size,
                                 worker_init_fn=seed_worker)
    ...
    for epoch in (range(num_epochs)):
        with torch.autograd.set_detect_anomaly(True):
            for i, (inputs_h, inputs_m, inputs_mask, labels) in enumerate(train_loader):
                if epoch == 0 and i == 0 and lambda_ > 0.0:
                    with torch.no_grad():
                        mu_temp, sigma_temp, xi_p_temp, xi_n_temp, yhat_temp = model(inputs_h, inputs_m, inputs_mask,
                                                                                     y_max, y_min, zero_tensor,
                                                                                     zero_tensor, zero_tensor,
                                                                                     zero_tensor, hetero)
                        mu_fix = mu_temp - mu_init
                        sigma_fix = sigma_temp - sigma_init
                        mu_temp, sigma_temp, xi_p_temp, xi_n_temp, yhat_temp = model(inputs_h, inputs_m, inputs_mask,
                                                                                     y_max, y_min, mu_fix, sigma_fix,
                                                                                     zero_tensor, zero_tensor, hetero)
                        xi_p_fix = xi_p_temp - xi_p_init
                        xi_n_fix = xi_n_temp - xi_n_init
                        print(
                            f'initial values: mu (mean): {mu_temp.mean().item()}, sigma (mean): {sigma_temp.mean().item()}, xi_p (mean): {xi_p_temp.mean().item()}, xi_n (mean): {xi_n_temp.mean().item()}')
                if lambda_ > 0.0:
                    mu, sigma, xi_p, xi_n, yhat = model(inputs_h, inputs_m, inputs_mask, y_max, y_min, mu_fix,
                                                        sigma_fix, xi_p_fix, xi_n_fix, hetero)
                    ...
                    mu, sigma, xi_p, xi_n, yhat = model(inputs_h, inputs_m, inputs_mask, y_max, y_min, zero_tensor,
                                                        zero_tensor, zero_tensor, zero_tensor, hetero)

                if lambda_ > 0.0:
                    constraint = 1 + (xi_p / sigma) * (labels - mu)
                    count_constraint_violation.append(constraint[constraint < small_value].shape[0])
                    gev_loss = calculate_nll(labels, mu, sigma, xi_p, is_return=True) / (labels.shape[0])
            ...
            if validation:
                for j, (inputs_h, inputs_m, inputs_mask, labels) in enumerate(validation_loader):
                    with torch.no_grad():
                        if lambda_ > 0.0:
                            mu, sigma, xi_p, xi_n, y_validation_predict = model(inputs_h, inputs_m, inputs_mask, y_max,
                                                                                y_min, mu_fix, sigma_fix, xi_p_fix,
                                                                                xi_n_fix, hetero)
                        else:
                            mu, sigma, xi_p, xi_n, y_validation_predict = model(inputs_h, inputs_m, inputs_mask, y_max,
                                                                                y_min, zero_tensor, zero_tensor,
                                                                                zero_tensor, zero_tensor, hetero)
                        rmse_loss = ((y_validation_predict - labels) ** 2).mean().sqrt()
                        validation_loss = rmse_loss
                validation_history[epoch] = validation_loss.item()

                for k, (inputs_h, inputs_m, inputs_mask, labels) in enumerate(test_loader):
                    with torch.no_grad():
                        if lambda_ > 0.0:
                            mu, sigma, xi_p, xi_n, y_test_predict = model(inputs_h, inputs_m, inputs_mask, y_max, y_min,
                                                                          mu_fix, sigma_fix, xi_p_fix, xi_n_fix, hetero)
                        else:
                            mu, sigma, xi_p, xi_n, y_test_predict = model(inputs_h, inputs_m, inputs_mask, y_max, y_min,
